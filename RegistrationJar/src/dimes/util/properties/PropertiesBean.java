...
public abstract class PropertiesBean implements Serializable {
	...
	public static void init(String propFileName)
	{
		...
		{
			e.printStackTrace();
			Loggers.getLogger().warn(e.toString());
		}
		try{
...
/*		String result=null;
	...
	private static Element getElement(String dottedPath, Element aRoot) throws NoSuchPropertyException
	{
		...
		if (elements.size() > 1)
		{
			Loggers.getLogger().debug("Property name conflict for property: " + dottedPath);
			...
			Loggers.getLogger(PropertiesBean.class).debug(msg);
		}
	...
	private static synchronized void manualMerge(Element oldRoot)
	{
		...
		{
			Loggers.getLogger(PropertiesBean.class).debug(e1.toString());
		...
		{
			Loggers.getLogger(PropertiesBean.class).warn(e.toString());
		}
