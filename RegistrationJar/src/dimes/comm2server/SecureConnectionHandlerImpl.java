...
//			URL serverKeyStroeRscr = this.getClass().getClassLoader().getResource(ResourceManager.SERVER_KEYSTORE_RSRC);
			...
			for (int i = 0; i < serverCerts.length; ++i)
			{
				...
				{
					/*System.err.println*/logger.warn("different certificates!");
					secConnection.disconnect();
					return /*null*/;
				}
				/*System.out.println*/logger.debug("same certificate!");
				try
				{
				...
				{
					e3.printStackTrace();
					/*System.err.println*/logger.warn("could not verify key!");
					secConnection.disconnect();
					return /*null*/;
				}
				/*System.out.println*/logger.debug("verified!");
			}
	...
	private class CertificateHostnameVerifier implements HostnameVerifier
	{
		...
		{
			/*System.out.println*/logger.debug("WARNING: Hostname is not matched for cert.");
			String sessionPeer = session.getPeerHost();
			hostNameFromCer = hostNameFromCer.trim();
			/*System.out.println*/logger.debug("certificate: " + hostNameFromCer + "\tsession: " + sessionPeer);
			try
			{
				...
				for (int i = 0; i < resolvedHostIPs.length; ++i)
				{
					...
					/*System.out.println*/logger.debug("session: " + sessionPeer + "\tcertificate: " + hostNameFromCer + "\tresolved IP from cer: "
							+ resolvedHostIPstr + "\tresolved name from cer: " + resolvedHostNameStr);
					if (sessionPeer.equalsIgnoreCase(resolvedHostIPstr))
					{
						/*System.out.println*/logger.debug("managed to resolve " + hostNameFromCer + " to IP: " + resolvedHostIPstr
								+ ". equals to session peer: " + sessionPeer);
						return true;
					}
					if (sessionPeer.equalsIgnoreCase(resolvedHostNameStr))
					{
						/*System.out.println*/logger.debug("managed to resolve " + hostNameFromCer + " to name: " + resolvedHostNameStr
								+ ". equals to session peer: " + sessionPeer);
						return true;
					}
				}
