...
public class UpdateDetailsHandler
{
	 ...
	 * @return
	 */
	public UpdateDetailsStatus changePassword(String currPassword, String newPdw)
	{

		UpdateDetailsStatus request = new UpdateDetailsStatus();
		request.oldPassword = currPassword;
		request.newPassword = newPdw;

		UpdateDetailsStatus stauts = this.generalUpdate(request);
		return stauts;
	}
	...
	//        return updateDetails(registrationString,header,trailer);
	//    }
	public UpdateDetailsStatus changeAutoUpdate(String anAutoUpdate)
	{
	 ...
	 * @return
	 */
	public UpdateDetailsStatus changeUserDetails(String userName, String country, String email, String agentName)
	{
		UpdateDetailsStatus details = new UpdateDetailsStatus();
	...
	public UpdateDetailsStatus changeAgentName(String agentName)
	{
		...
		UpdateDetailsStatus status = generalUpdate(details);
		return status;
	}
	 ...
	 * @return
	 */
	public UpdateDetailsStatus changeGroupDetails(String group, String groupAction)
	{
	 ...
	 */
	private Element updateDetails(String updateFileString, String header, String trailer) throws IOException, MalformedURLException
	{
		StringWriter serverResponse = new StringWriter();
		BufferedWriter bfrWriter = new BufferedWriter(serverResponse);
		BufferedReader regStrBuffer = new BufferedReader(new StringReader(updateFileString));

		logger.finest("Sending: " + updateFileString);

		// exchange files :
		comm.exchangeFiles(regStrBuffer, bfrWriter, header, trailer);
			
		logger.finest("Got in return :" + serverResponse.toString());

		// read and update the display :
//		StringReader serverResponseReader = new StringReader(serverResponse.toString());
//		SAXReader iregDocReader = new SAXReader();
//		Document result = iregDocReader.read(serverResponseReader);
		return XMLUtil.getRootElement(serverResponse.toString());
	}
	 ...
	 * @param result2
	 */
	private UpdateDetailsStatus readUpdateToResult(Element resultDoc, UpdateDetailsStatus generalRequest)
	{

		UpdateDetailsStatus result = new UpdateDetailsStatus();
		/*System.out.println*/logger.finest("Reading update to result...");
		Element rootElem = resultDoc;
		...
		if (userElement != null)
		{
			...
			else
			{
				...
				result.hasPassword = (XMLUtil.getChildElementByName(userElement, "password").getTextContent()==null)? null : "true";//(userElement.element("password") == null) ? null : "true";
		...
		if (groupElement != null)
		{
			...
			{
				result.groupName = XMLUtil.getChildElementByName(groupElement,"groupName").getTextContent();//groupElement.elementText("groupName");
				result.groupOwner = XMLUtil.getChildElementByName(groupElement,"groupOwner").getTextContent();//groupElement.elementText("groupOwner");
	 ...
	 * @return
	 */
	public UpdateDetailsStatus generalUpdate(UpdateDetailsStatus generalRequest)
	{
		String currentAgentName = null;
		...
		catch (MalformedURLException e1)
		{
			...
			Loggers.getLogger().warning("Internal Agent error :\n" + e1.getMessage());
		...
		{
			// communication error :
			Loggers.getLogger().warning("Error while connecting server : " + e1.getMessage());
			result.communicationError = true;
			result.errorMessage = "Error while connecting server: \n " + e1.getMessage()
	 ...
	 * @return
	 */
	private String generateRequestString(UpdateDetailsStatus generalRequest)
	{
		String registrationString = "\t<update-details>\n";
		// Write the agent details :                      
	 ...
	 * @return
	 */
	public UpdateDetailsStatus validateAgentDetails(String agentID)
	{
		/*System.out.println*/logger.finest("validating agent details. agent name is :" + agentID);
