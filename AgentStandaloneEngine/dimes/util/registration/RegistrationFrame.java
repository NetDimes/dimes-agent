...
public class RegistrationFrame extends JFrame implements UpdateDetailsParentFrame, RegisterDetailsParentFrame
{
	...
	private static final String REGISTRATION_SUCCESS_HTML_LABEL = "<html>" + "<body>" + "<p align=\"left\">" + "<font color=#113355 face=\"Arial\" size=\"4\">"
			...
			+ " Dimes Agent version 0.5.2 was successfuly registered.<br>" + " Look for the small cactus icon on the system tray. <br><br>"
			+ " Many new features are included:" + " <ul>" + " <li> Personal Agents and User Statistics Panel" + " <li> A Better Agent's Files Management System" + " <li>Thinner Executable"
			+ " <li> Automatic Update of Net Graph Data (IPs and Countries)" + " <li> and many more..." + " </ul>" + " " + "</font>" + "</p>" + "</body>" + "</html>";
	 ...
	 * @since 0.5.0
	 */
	public RegistrationFrame(StandardCommunicator comm, boolean agentIDAlreadyExist, Agent theAgent) {
	 ...
	 */
	public RegistrationFrame(AgentFrameFacade frame, StandardCommunicator comm, boolean agentIDAlreadyExist)
	{
		...
		if (agentWasWithValidID)
		{
			...
			{
				Loggers.getLogger().severe("Agent informed having agentID without having one. Resuming regular registration");
				agentWasWithValidID = false;
				this.currentScreen = RegistrationFrame.USER_DECISION_SCREEN;
			}
		}
		myParentAgentFrame = frame;
		communicator = comm;
	...
	public void jbInit()
	{
...

		{
			...
			{ //todo - should add the property if doesn't exist
				Loggers.getLogger().fine(e1.toString()); //debug
...
//		screen7.show();
	...
	protected void finishRegistration()
	{
		if( myParentAgentFrame != null )
			this.myParentAgentFrame.applyRegistrationSuccess();
		else
	...
	private void nextRegistrationPhase()
	{
		...
		switch (this.currentScreen)
		{
			...
			case RegistrationFrame.VETERAN_AGENTS_SCREEN :
			{
				...
				{
					/*System.out.println*/logger.finest("selected agent: " + selectedAgent);//debug
					if (regMonitorPanel != null)
						regMonitorPanel.stopProgress();
			...
			case RegistrationFrame.REGISTRATION_SUCCESS_SCREEN :
				if( myParentAgentFrame!= null )
					...
					theAgent.applyRegistrationSuccess();
				this.dispose();
				break;
			...
			case RegistrationFrame.VALIDATION_RESULT_SCREEN :
			{
				if (regMonitorPanel != null)
					regMonitorPanel.stopProgress();
				UpdateDetailsStatus updateRequest = getValidationUpdateDetails(); // retuns null if no changes were done.
	 ...
	 */
	private UpdateDetailsStatus getValidationUpdateDetails()
	{
		UpdateDetailsStatus validationDetails = new UpdateDetailsStatus();
		...
		{
			Loggers.getLogger().warning("Can't find country property.");
		...
		{
			Loggers.getLogger().warning("can't find email property");
		...
		{
			Loggers.getLogger().severe("could not find critical property:" + e2.getMessage());
			return null;
		}
	 ...
	 * @param updateStat
	 */
	public void returnUpdateState(UpdateDetailsStatus updateStat)
	{
		if (regMonitorPanel != null)
			regMonitorPanel.stopProgress();
		/*System.out.println*/logger.finest("returned with update. current screen is :" + currentScreen);
	 ...
	 * @param regStat
	 */
	public void returnRegistrationState(RegistrationStatus regStat)
	{
		if (regMonitorPanel != null)
			regMonitorPanel.stopProgress();
		if (regStat.errorMessage != null)
		{
			...
			{
				/*System.err.println*/logger.fine("error: " + regStat.errorMessage);//debug
	 ...
	 * @param regStat
	 */
	private void setRegistrationProperties(RegistrationStatus regStat)
	{
		try
		{
			if( this.myParentAgentFrame!= null )
				this.myParentAgentFrame.writeRegistrationDetails(regStat);
			else
				this.theAgent.writeRegistrationDetails(regStat);
		}
		catch (IOException e)
		{
	 ...
	 * @param updateStat
	 */
	private void setUpdateProperties(UpdateDetailsStatus updateStat)
	{
		if (!myParentAgentFrame.writeUpdatedDetails(updateStat))
			JOptionPane.showMessageDialog(this, "Could not update Properties file.\n Please check properties.xml", "DIMES Error", JOptionPane.ERROR_MESSAGE);
	}
