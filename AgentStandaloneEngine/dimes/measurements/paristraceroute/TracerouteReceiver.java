...
import dimes.measurements.basicmeasurements.NetHostLocal;
...
public class TracerouteReceiver implements CallbackContext {
	...
	public TracerouteReceiver(short maxRouteLength, NetworkStack myStack) {
	...
	public boolean callback(byte[] p, long milisecReceiveTime, long microsecReceiveTime) {
		...
		try 
		{
			...
			int ipID = receivedPacket.getHeader().getHeaderId(null);
//			int protocol = receivedIpHeader.getNextHeaderId();
	...
	public Vector<NetHost> getQBEResults() {
		...
		do {
			...
			while (routeItr1.hasNext()) {
				NetHostLocal nhl = routeItr1.next();
	...
	public Vector<NetHost> getPingTracerouteResults() {
		...
		while (cycleItr.hasNext()) {
			...
			// merge
			Iterator<NetHostLocal>  routeItr2 = route2.iterator();
			Iterator<ComplexNetHost> mainRouteItr = results.iterator();
			while (routeItr2.hasNext()) {
				if (mainRouteItr.hasNext()) {
					NetHostLocal nhl2 = (NetHostLocal) routeItr2.next();
					ComplexNetHost nh = mainRouteItr.next();
					if (nh.getHopAddressStr().equalsIgnoreCase(nhl2.getHostAddress())) {
