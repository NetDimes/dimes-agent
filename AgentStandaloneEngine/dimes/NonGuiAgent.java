...
import dimes.AgentGuiComm.logging.RemoteLogHandler;
...
public class NonGuiAgent implements HeaderProducer, Agent
{
...
//	private String propFileName; //properties
	private static final Logger logger = Loggers.getLogger(NonGuiAgent.class);	//logging
...
//	private UpdateDetailsThread updateDetailsThread;
	...
	private RemoteLogHandler remoteCommHandler;
//	private GraphHandler graphHandler;
	private ResultSenderHandler resultSenderHandler;
	private static NonGuiAgent me=null;
	private static boolean debug = false;
//	private AgentEngineTester agentEngineTester= null;
	
	public static NonGuiAgent getInstance() throws MalformedURLException, NoSuchPropertyException{
		if (null==me) me=new NonGuiAgent(null);
		return me;
	}
	
	public static Agent getInstance(AgentFrameFacade theAgentFrame) throws MalformedURLException, NoSuchPropertyException{
		if (null==me) me=new NonGuiAgent(theAgentFrame);
		return me;
	}
	 ...
	 * @since 0.5.0
	 */
	private NonGuiAgent() throws MalformedURLException, NoSuchPropertyException {
		this(null);
	 ...
	 * @throws NoSuchPropertyException
	 */
	private NonGuiAgent(AgentFrameFacade theAgentFrame) throws MalformedURLException, NoSuchPropertyException
	{
		...
		NonGuiAgent.VERSION_STR = PropertiesBean.getProperty(PropertiesNames.AGENT_VERSION); 	//get version from properties.xml		
		remoteCommHandler = new RemoteLogHandler();
...
//		Logger resultWriter = Logger.getLogger(resultWriterName);
		...
		NonGuiAgent.logger.info("Creating agent ver." + VERSION_STR + " GUI not available on this system");//debug
		...
		this.status = new Status(NonGuiAgent.VERSION_STR, this.sched.getTaskManagerInfo());//check
...
//		DisplayUpdateHandler displayUpdateHandler = new DisplayUpdateHandler(server);
		...
		Logger resultWriter = Logger.getLogger(resultWriterName);
		resultSenderHandler = ResultSenderHandler.getInstance();//agentFrame);
...
//		System.out.println(agentEngineTester.testPrintSystaxTree()); 
	...
	public void debugMode(/*boolean debug*/)
	{
		...
		{
			NonGuiAgent.logger.warning(e.toString());//debug
		}
		catch (Exception e)
		{
			NonGuiAgent.logger.warning(e.toString());//debug
...
//			long period = this.schedulerTask.getPeriod();
	...
	public void restart()
	{
		...
		NonGuiAgent.logger.fine("property is :" + System.getProperty(AppSplash.INTERNAL_RESTART_PROPERTY));
		...
		while(!Boolean.valueOf(System.getProperty(AppSplash.INTERNAL_RESTART_PROPERTY)).booleanValue()){ //Wait until we're sure that the Internal property is set. 
			NonGuiAgent.logger.fine("Waiting for INTERNAL_RESTART_PROPERTY"+iter); //debug 
		...
		NonGuiAgent.logger.fine("Agent frame Restart = " + restart);
	...
	public boolean isRegistered(String registeredProp)
	{
		...
		{
			NonGuiAgent.logger.fine(e.toString());//debug
			return false;
		}
...
*/
	...
	{
		try {
			String propertiesDir= PropertiesBean.getProperty(PropertiesNames.BASE_DIR)+File.separator+"conf"+File.separator;
			File regFile = new File(propertiesDir+"registration.xml");
//			Registrator reg = Registrator.getInstance(propertiesDir+File.separator+"properties.xml");
			if(regFile.exists()){
...
//		NonGuiAgent.logger.exiting("dimes.Agent", "register");//debug
	}
...
/*	private void register(StandardCommunicator registrationComm, boolean agentIDAlreadyExist)
	...
	public boolean blockedProtocolsWereChecked()
	{
		...
		{
			NonGuiAgent.logger.warning(e2.toString());
		}
	...
	public void checkBlockedProtocols()
	{
		...
		{
			NonGuiAgent.logger.warning(e.toString());
		}
		...
		{
			NonGuiAgent.logger.warning(e1.toString());
		}
			...
			switch (defaultProto)
			{
				case Protocol.UDP :
					if (udpBlocked)
						...
						{
							NonGuiAgent.logger.warning(e3.toString());
						}
					break;
				case Protocol.ICMP :
					if (icmpBlocked)
	...
	public boolean handleRegistration() throws IOException, NoSuchPropertyException
	{
		...
		{
			NonGuiAgent.logger.fine("not registered");//debug
	...
	public boolean agentIDExist() { 
		...
		{
			Loggers.getLogger().info("Agent ID Property not found : initiating Clean installation process.");
			return false;
		}
...
//	}
	 ...
	 */
/*	public boolean writeUpdatedDetails(PropertiesStatus updateStat)
	{
		boolean success = true;
		try
		{
